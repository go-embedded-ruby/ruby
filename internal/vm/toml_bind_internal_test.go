...
func TestTOMLToBridge(t *testing.T) {
	...
	if v, ok := toTOML(unixTime(42)).(stdtime.Time); !ok || v.Unix() != 42 {
		t.Errorf("time -> %T", toTOML(unixTime(42)))
	}
...
func TestTOMLFromBridge(t *testing.T) {
	...
	if tm, ok := fromTOML(vm, off).(*Time); !ok || tm.t.Unix() != 296638320 {
	...
	if tm, ok := fromTOML(vm, ldt).(*Time); !ok || tm.t.Unix() != want {
		t.Errorf("local dt -> %#v", fromTOML(vm, ldt))
	}
	// Local date.
	ld := toml.LocalDate{Year: 1979, Month: 5, Day: 27}
	if tm, ok := fromTOML(vm, ld).(*Time); !ok || tm.t.Unix() != stdtime.Date(1979, 5, 27, 0, 0, 0, 0, stdtime.UTC).Unix() {
		t.Errorf("local date -> %#v", fromTOML(vm, ld))
	}
	// Local time (on the epoch date).
	lt := toml.LocalTime{Hour: 7, Minute: 32, Second: 5}
	if tm, ok := fromTOML(vm, lt).(*Time); !ok || tm.t.Unix() != stdtime.Date(1970, 1, 1, 7, 32, 5, 0, stdtime.UTC).Unix() {
