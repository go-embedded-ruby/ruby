...
type VM struct {
	...
	// liveObjs / liveClasses back ObjectSpace.each_object: weak pointers to every
	// *RObject / *RClass allocated through the ordinary constructors (Class#new,
	// #allocate, Class.new, Module.new). They are weak so the registry never keeps
	// an object alive — a collected entry resolves to nil and is compacted away on
	// the next each_object sweep. rbgo has no global heap table, so this is a
	// best-effort view scoped to those constructor paths (see objectspace.go).
	// GVL-guarded.
	liveObjs    []weak.Pointer[RObject]
	liveClasses []weak.Pointer[RClass]

	// finalizers records ObjectSpace.define_finalizer registrations, keyed by the
	// target object's identity id (refID). The Go GC owns reclamation, so these
	// never actually run; the table exists so define_finalizer can dedup by == and
	// undefine_finalizer can clear it, matching MRI's observable return values.
	// GVL-guarded.
...
func (vm *VM) exec(iseq *bytecode.ISeq, self object.Value, args []object.Value, definee *RClass, methodName string, parentEnv *Env, block, selfBlock, blockArg *Proc, methodLexScope *RClass) (execResult object.Value) {
	...
	// TracePoint's exit event (:return / :b_return / :end) for a frame that does
	// NOT finish: an exception, a throw, or a non-local return passing through.
	//
	// MRI raises it there too — hook_before_rewind fires RETURN as it rewinds the
	// frames an exception is unwinding past — and reports #return_value as nil,
	// because the frame produced none. `def a2; raise; end` called from a1 under
	// a :return TracePoint reports [:return, :a2, nil] then [:return, :a1, nil],
	// and rbgo reported neither until this defer existed.
	//
	// It is registered BEFORE the returnSignal recover below so that it RUNS
	// AFTER it (defers are LIFO): the recover raises the event itself, with the
	// returned value, and sets exitFired so this one stays quiet. exitFired is
