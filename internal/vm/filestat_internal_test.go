...
func TestFileStatSpaceship(t *testing.T) {
	...
	if got := cmp.native(vm, older, []object.Value{newer}, nil); got != object.Integer(-1) {
		t.Errorf("older <=> newer: got %v want -1", got)
	}
	if got := cmp.native(vm, newer, []object.Value{older}, nil); got != object.Integer(1) {
		t.Errorf("newer <=> older: got %v want 1", got)
	}
	if got := cmp.native(vm, older, []object.Value{older}, nil); got != object.Integer(0) {
